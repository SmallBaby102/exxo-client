...
class Dashboard extends React.Component {
  ...
  finish = () => {
    ...
    formData.append("country", this.state.country?.name)
  ...
  componentDidMount() {
    ...
    this.setState({ country: { ...country, label: country?.name,  value: country?.isoCode},  state: { label: state?.name,  value: state?.isoCode, ...state }, city: { ...city, label: city?.name, value: city?.name} })
    if(account?.birthday){
